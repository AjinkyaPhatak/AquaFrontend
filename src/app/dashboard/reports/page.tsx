...
export default function ReportsPage() {
  ...
  const generateReportHTML = (report: WaterAnalysis) => {
    ...
    return `
      ...
      <head>
        <meta charset="UTF-8">
        <title>Lake Frothing Report - ${report.waterType || 'Analysis'}</title>
        <style>
      ...
      <body>
        <div class="container">
          <div class="header">
            <div class="logo">💧</div>
            <h1>Lake Frothing Forecast Report</h1>
            <p>AquaSense AI-powered frothing prediction from satellite imagery</p>
          </div>
          ...
          <div class="section">
            <div class="section-title">⚠️ Likely Drivers</div>
          ...
          <div class="footer">
            <p>Generated by AquaSense Lake Frothing Forecast System</p>
  ...
  const shareViaWhatsApp = (report: WaterAnalysis) => {
    const message = `🌊 *Lake Frothing Report*\n\n` +
      `📊 *Risk Score:* ${report.overallSafetyScore.toFixed(2)}/100\n` +
      ...
      Object.entries(report.parameters).map(([key, param]) => {
        const names: Record<string, string> = {
          foamCoverage: 'Foam', algaeDensity: 'Algae', shorelineResidue: 'Residue',
          waterDiscoloration: 'Color', stagnationIndex: 'Stagnation', surfaceVolatility: 'Volatility'
        };
  ...
  const shareViaEmail = (report: WaterAnalysis) => {
    const subject = `Lake Frothing Report - ${report.waterType || 'Analysis'} - Score: ${report.overallSafetyScore.toFixed(2)}`;
    const body = `Lake Frothing Forecast Report\n\n` +
      `Risk Score: ${report.overallSafetyScore.toFixed(2)}/100\n` +
      ...
      Object.entries(report.parameters).map(([key, param]) => {
        const names: Record<string, string> = {
          foamCoverage: 'Foam Coverage', algaeDensity: 'Algae Density', shorelineResidue: 'Shoreline Residue',
          waterDiscoloration: 'Water Discoloration', stagnationIndex: 'Stagnation Index', surfaceVolatility: 'Surface Volatility'
        };
        return `- ${names[key] || key}: ${param.value.toFixed(2)} (${param.status})`;
      }).join('\n') +
      `\n\n${report.detailedAnalysis || ''}\n\n` +
      `Generated by AquaSense Lake Frothing Forecast System`;
    
    window.location.href = `mailto:?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
  };

  const copyReportLink = async (report: WaterAnalysis) => {
    const reportText = `Lake Frothing Report\n` +
  ...
  const getParameterIcon = (key: string) => {
    const icons: Record<string, React.ReactNode> = {
      foamCoverage: <Droplets className="w-4 h-4" />,
      algaeDensity: <Leaf className="w-4 h-4" />,
      shorelineResidue: <Waves className="w-4 h-4" />,
      waterDiscoloration: <Beaker className="w-4 h-4" />,
      stagnationIndex: <ThermometerSun className="w-4 h-4" />,
      surfaceVolatility: <AlertTriangle className="w-4 h-4" />,
    };
    ...
    >
      ...
      >
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400">View, export, and share your lake frothing forecast reports</p>
        </div>
            ...
            >
              ...
              <div className={`p-6 border-b ${getStatusBg(selectedReport.safetyStatus)}`}>
                ...
                <div className="mt-6 flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 mt-1">Risk Score</p>
              ...
              <div className="p-6 space-y-6">
                ...
                {selectedReport.potentialContaminants && selectedReport.potentialContaminants.length > 0 && (
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">Likely Drivers</h3>
